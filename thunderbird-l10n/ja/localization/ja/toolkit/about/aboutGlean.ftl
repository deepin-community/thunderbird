...
about-glean-profiler-explanation-profiler = パフォーマンスプロファイルでは、収集したすべてのメトリクス、収集した時刻、収集した正確な値を確認できます。それぞれのマーカーにカーソルを合わせると、正確な値が適切なタイミングで収集されているか検証できます。
...
about-glean-about-data-description = 確認したいデータに合わせて、いくつか別のツールがあります。
about-glean-about-data-description-list-intro = 特定のユースケースのためのツールは、下記のリストを参照してください:
about-glean-about-data-list-item-dictionary = { -glean-brand-name } でアプリケーションごとに収集したデータのリストの閲覧は、<a data-l10n-name="glean-dictionary-link">{ -glean-brand-name } Dictionary</a> で調べてください。
about-glean-about-data-list-item-about-telemetry = 従来のテレメトリーで収集したデータの閲覧は、<a data-l10n-name="about-telemetry-link">about:telemetry</a> で調べてください。
about-glean-about-data-list-item-debug-ping-viewer = デバッグタグの閲覧や完全な Ping の確認、ライブイベントストリームの確認、視覚的なメトリクスの表示は、<a data-l10n-name="glean-debug-ping-viewer">{ glean-debug-ping-viewer-brand-name }</a> で調べてください。
about-glean-about-data-list-item-firefox-profiler = パフォーマンスプロファイルの記録と、記録したすべてのメトリクスの確認は <a data-l10n-name="about-glean-firefox-profiler">{ -profiler-brand-name }</a> を使用してください。
